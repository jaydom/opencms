...
router.get('/:table_name/all', function(req, res, next) {
    var table_name = req.params["table_name"];
    var conditions = {"delete_date":null};
...
router.delete('/:table_name/:id', function(req, res, next) {
    ...
    var table_name = req.params["table_name"];
    req.models[table_name].find({id:req.params.id}).each(function (data) {
        ...
        if(err){
            ...
            res.statusCode = 200;
            res.json({success:true, messag: "delete success"});
        }
    });
});
...
router.post('/:table_name/delete_all', function(req, res, next) {
    var now = new Date().toLocaleString();
    var table_name = req.params["table_name"];
    var condition = {"delete_date":null};
    ...
    req.models[table_name].find(condition).each(function (data) {
        data["delete_date"] = now;
    }).save(function (err) {
        if(err){
            res.statusCode = 400;
...
router.put('/:table_name', function(req, res, next) {
    ...
    req.db.driver.execQuery("SELECT seqid_generate_v1('test')", function (err, data) {
        console.log(data[0]["seqid_generate_v1"]);
    })
...
router.get('/:table_name/:id', function(req, res, next) {
    ...
    req.models[table_name].find({id:req.params.id,delete_date:null}).run(function(err, users){
        if(err){
            //res.send(err);
            console.log(err);
            res.json([]);
...
router.get('/result', function(req, res, next) {
    ...
    req.models.Certificate.find({ player_name: req.query.user_name,delete_date:null }).run(function(err, users){
        if(err){
            //res.send(err);
            console.log(err)
        }else{
        ...
        if(err){
            ...
            data.save(function (err) {
                if(err){
                    ...
                    req.models[map_table_name].find({referee_group_id:req.body.id}).each(function (data) {
                        data["delete_date"] = now;
                    }).save(function (err) {
                        if(err){
                            res.statusCode = 400;
...
router.delete('/:table_name/group/:map_table_name/:id', function(req, res, next) {
    ...
    req.models[map_table_name].find(condition).each(function (data) {
        data["delete_date"] = now;
    }).save(function (err) {
        if(err){
            //res.send(err);
...
router.post('/:table_name/group/:map_table_name/delete_all', function(req, res, next) {
    ...
    req.models[map_table_name].find(condition_map_data).each(function (data) {
        data["delete_date"] = now;
    }).save(function (err) {
        if(err){
            res.statusCode = 400;
